#!/bin/bash
# ==========================================================
# menus/vmess_ws_menu.sh — VMess + WebSocket + TLS
# Imagitech XRAY Suite
# Nginx proxies public port 443 → internal 10003
# Users are identified by UUID; alterId: 0
# ==========================================================
...
INBOUND_TAG="vmess-ws-tls"
USERS_PROTO="vmess-ws-tls"
LOCAL_PORT=10003          # Nginx → Xray internal port
...
# ==========================================================
# Internal helper: add a VMess client via jq
# Usage: vmess_add_client TAG UUID REMARK
# VMess client schema: {"id": UUID, "alterId": 0, "email": REMARK}
# ==========================================================
vmess_add_client() {
    local tag="$1"
    local uuid="$2"
    ...
    client_json=$(jq -n \
        --arg id     "$uuid" \
        --arg email  "$remark" \
        '{"id":$id,"alterId":0,"email":$email,"level":0}')
...
# Internal helper: remove a VMess client by UUID
# Usage: vmess_del_client TAG UUID
vmess_del_client() {
    local tag="$1"
    local uuid="$2"
...
add_user() {
    ...
    if jq -e --arg t "$INBOUND_TAG" \
        ...
        vmess_add_client "$INBOUND_TAG" "$uuid" "$remark"
        xray_reload
    else
...
del_user() {
    ...
    vmess_del_client "$INBOUND_TAG" "$SELECTED_USER_ID"
    xray_reload
...
show_user_link() {
    ...
    # build_vmess_ws_link produces the vmess://base64 URI
    local link
    link=$(build_vmess_ws_link \
        "$SELECTED_USER_ID" \
        "$DOMAIN" \
        "$NGINX_PORT" \
        "$WS_PATH" \
...
EOF
    ...
    if [[ -f "$uf" && -s "$uf" ]]; then
        ...
        while IFS='|' read -r uid uremark _; do
            vmess_add_client "$INBOUND_TAG" "$uid" "$uremark"
        done < "$uf"
    fi
