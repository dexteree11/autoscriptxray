#!/bin/bash
# ==========================================================
# menus/trojan_ws_menu.sh — Trojan + WebSocket + TLS
# Imagitech XRAY Suite
# Nginx proxies public port 443 → internal 10002
...
INBOUND_TAG="trojan-ws-tls"
USERS_PROTO="trojan-ws-tls"
LOCAL_PORT=10002          # Nginx → Xray internal port
...
add_user() {
    ...
    # Generate Trojan password (random base64 string, not UUID)
    local password
    password=$(gen_trojan_password)
    success "Generated Password: ${BWHITE}${password}${NC}"

    # Persist to user file (password used as ID)
...
del_user() {
    ...
    # For Trojan, SELECTED_USER_ID holds the password
...
EOF
    ...
    # Re-add all existing users (password is the ID field)
    local uf
